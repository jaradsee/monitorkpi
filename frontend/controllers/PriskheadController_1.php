...
$this->removeUploadDir($model->ref);
   ...
   public function actionGetLevel() {
    ...
    public function sendLine ($model)
    {
        ...
        if(!empty(Yii::$app->faktharm->getFirstImage($model->description))){
           curl_setopt($ch,CURLOPT_POSTFILDS,"masseage=" .$model->name."&imageTumbnail=".Yii::$app->faktharm->getFirstImage($model->description));
        }else{
           curl_setopt($ch,CURLOPT_POSTFILDS,"masseage=" .$model->name);

        }
        //follow redirects
        curl_setopt($ch,CURLOPT_FOLLOWLOCATION, 1);
        curl_setopt($ch,CURLOPT_HTTPHEADER, [
            'Content-type: application/x-www-form-urlencoled',
